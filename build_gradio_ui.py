"""
Rebuilds quran_expert_system_gradio.ipynb with:
- soundfile for MP3 loading (works in python-starter env)
- Full traceback shown in UI on error so we can see what fails
- Float->int fix for gr.Number inputs
"""
import json
from pathlib import Path

NB_PATH = Path(__file__).parent / "quran_expert_system_gradio.ipynb"
...
nb = {
    "cells": [
        md_cell(["# 📖 TasmiqAI — Quran Expert System (Gradio)\n\n",
                 "Run **Cell 1** once → **Restart kernel** → Run **Cell 2**."]),
        md_cell(["## Cell 1 — Install"]),
        INSTALL,
        md_cell(["## Cell 2 — Launch UI\n",
                 "Loads dataset + model, opens Gradio in browser."]),
        APP,
    ],
    "metadata": {
        "kernelspec": {
            "display_name": "Python (python-starter)",
            "language": "python",
            "name": "python-starter"
        },
        "language_info": {
            ...
            "name": "python", "pygments_lexer": "ipython3", "version": "3.10.0"
        }
    },
    "nbformat": 4,
    "nbformat_minor": 4
