...
const Field = ({ icon, placeholder, value, onChangeText, secure, keyboardType, extra, showPassword, C, autoComplete = 'off' }) => (
  <View style={{
    ...
    <Ionicons name={icon} size={20} color={'#0B6E4F'} style={{ marginRight: 14 }} />
    <TextInput
      value={value}
      ...
      style={{ flex: 1, fontSize: 16, color: '#064E3B' }}
    />
    {extra}
  </View>
);
...
export default function SignUpScreen({ navigation, route }) {
  ...
  const ACCENT = isStaff ? '#C8B6E2' : '#0B6E4F';
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFDF0' }}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', paddingHorizontal: 28, paddingVertical: 32, maxWidth: 500, alignSelf: 'center', width: '100%' }} showsVerticalScrollIndicator={false}>
          ...
          {/* Back Button */}
          <TouchableOpacity onPress={() => navigation.goBack()} style={{ marginBottom: 24, width: 44, height: 44, borderRadius: 22, backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center', elevation: 2 }}>
            <Ionicons name="arrow-back" size={24} color={'#064E3B'} />
          </TouchableOpacity>

          {/* Logo + Title */}
          <View style={{ alignItems: 'center', marginBottom: 40 }}>
            <View style={{ backgroundColor: '#FFFFFF', padding: 12, borderRadius: 24, elevation: 4, marginBottom: 20 }}>
              <Image
            ...
            <Text style={{ fontSize: 28, fontWeight: '900', color: '#064E3B', marginBottom: 8 }}>
          ...
          {success ? (
            <View style={{ backgroundColor: '#EDFAF4', borderRadius: 14, padding: 16, marginBottom: 20, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              ...
              <Text style={{ color: '#0B6E4F', fontSize: 14, flex: 1, fontWeight: '700' }}>{success}</Text>
            </View>
          ) : null}

          {/* Form Fields */}
          <Text style={{ fontSize: 13, fontWeight: '800', color: '#0B6E4F', marginBottom: 8, marginLeft: 4, textTransform: 'uppercase', letterSpacing: 1 }}>Full Name</Text>
          ...
          <Text style={{ fontSize: 13, fontWeight: '800', color: '#0B6E4F', marginBottom: 8, marginLeft: 4, textTransform: 'uppercase', letterSpacing: 1 }}>Email Address</Text>
          ...
          {email.toLowerCase().includes('tahfiz.my') && (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: -8, marginBottom: 16, marginLeft: 4 }}>
              <Ionicons name="shield-checkmark" size={14} color={'#0B6E4F'} />
              <Text style={{ fontSize: 12, color: '#0B6E4F', fontWeight: '700' }}>
                Verified {email.includes('staff') ? 'Staff' : 'Student'} Domain Detected
              </Text>
            </View>
          )}

          <Text style={{ fontSize: 13, fontWeight: '800', color: '#0B6E4F', marginBottom: 8, marginLeft: 4, textTransform: 'uppercase', letterSpacing: 1 }}>Password</Text>
          <Field
          ...
          <Text style={{ fontSize: 13, fontWeight: '800', color: '#0B6E4F', marginBottom: 8, marginLeft: 4, textTransform: 'uppercase', letterSpacing: 1 }}>Confirm Password</Text>
